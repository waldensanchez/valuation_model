...
def in_both_lists(list_1:list, list_2:list):
    """" If one list is bigger passing it to list_2 """
...
def clean_df(balance_statement: pd.DataFrame, income_statement: pd.DataFrame, sp500: list, prices_fiscal: list):
    ...
    balance_cols = ['fiscalDateEnding','currentDebt','inventory','totalAssets','totalCurrentAssets','currentAccountsPayable','currentNetReceivables','commonStockSharesOutstanding','totalLiabilities','totalShareholderEquity']
...
def CCC(data_table: pd.DataFrame):
    ...
    data_table: DataFrame resultant from using tabular_df function.
    """ 
    days_sales_of_inventory = 365 / ( data_table['costofGoodsAndServicesSold'] / data_table['inventory'] )
    days_sales_outstanding = 365 / ( data_table['totalRevenue'] / data_table['currentNetReceivables'] )
    days_payables_outstanding = 365 / ( data_table['costofGoodsAndServicesSold'] / data_table['currentAccountsPayable'] )
...
def financial_ratios(data_table: pd.DataFrame):
    ...
    return financials[['Stock','fiscalDateEnding','PER','PBV','Acid_test','ATR','CCC','ROA','DER','NPM','EM']]
